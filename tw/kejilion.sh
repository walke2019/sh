...
add_sshkey() {
	...
	ip_address
	echo -e "私钥信息已生成，务必复制保存，可保存成 ${gl_huang}${ipv4_address}_ssh.key${gl_bai}文件，用於以後的SSH登錄"
...
dd_xitong() {
		...
		dd_xitong_1() {
		  echo -e "重装后初始用户名: ${gl_huang}root${gl_bai}初始密碼:${gl_huang}LeitboGi0ro${gl_bai}  初始端口: ${gl_huang}22${gl_bai}"
		  echo -e "按任意鍵繼續..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
		...
		dd_xitong_2() {
		  echo -e "重装后初始用户名: ${gl_huang}Administrator${gl_bai}  初始密码: ${gl_huang}Teddysun.com${gl_bai}初始連接埠:${gl_huang}3389${gl_bai}"
		  echo -e "按任意鍵繼續..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
...
format_partition() {
	...
	# 格式化分割區
	echo "正在格式化分割區 /dev/$PARTITION 为 $FS_TYPE ..."
	...
	while true; do
		...
		case "$Limiting" in
		   ...
		   24) # 复制文件目录
				...
				# 使用 -r 选项以递归方式复制目录
				cp -r "$src_path" "$dest_path" && echo "檔案或目錄已複製到$dest_path" || echo "复制文件或目录失败"
				send_stats "复制文件或目录"
				;;
