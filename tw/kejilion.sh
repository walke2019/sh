...
ldnmp_Proxy_backend_stream() {
	...
	# 取得監聽埠
	read -rp "請輸入本機監聽埠 (如 3306):" listen_port
...
EOF
	...
	# 輸出產生的信息
	echo "服务 $service_name已成功加入到 frpc.toml"
...
yt_menu_pro() {
	...
	while true; do
		...
		echo -e "官網網址：https://github.com/yt-dlp/yt-dlp"
		echo "-------------------------"
		echo "已下載影片清單:"
...
correct_ssh_config() {
	...
	# 如果找到 PasswordAuthentication 設定為 yes
	...
	# 如果 PasswordAuthentication 和 PubkeyAuthentication 都沒有匹配，則設定預設值
...
import_sshkey() {
	...
	rm -rf /etc/ssh/sshd_config.d/* /etc/ssh/ssh_config.d/*
	restart_ssh
	echo -e "${gl_lv}公鑰已成功匯入，ROOT私鑰登入已開啟，已關閉ROOT密碼登錄，重連將會生效${gl_bai}"
...
rm -rf /etc/ssh/sshd_config.d/* /etc/ssh/ssh_config.d/*
restart_ssh
echo -e "${gl_lv}ROOT登入設定完畢！${gl_bai}"
...
linux_tools() {
  ...
  while true; do
	  ...
	  echo -e "${gl_kjlan}9.   ${gl_bai}tmux 多路後台運行工具${gl_kjlan}10.  ${gl_bai}ffmpeg 视频编码直播推流工具"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}11.  ${gl_bai}btop 现代化监控工具 ${gl_huang}★${gl_bai}             ${gl_kjlan}12.  ${gl_bai}ranger 檔案管理工具"
...
docker_ssh_migration() {
	...
	backup_docker() {
		...
		if [ -d "/home/docker" ]; then
			...
			echo -e "${GREEN}/home/docker 下的檔案已打包到:${BACKUP_DIR}/home_docker_files.tar.gz${NC}"
		fi
	...
	restore_docker() {
		...
		for f in "$BACKUP_DIR"/backup_type_*; do
			...
			if grep -q "compose" "$f"; then
				...
				echo -e "${GREEN}Compose 項目 [$project_name] 已解壓縮到:$original_path${NC}"

				cd "$original_path" || return
				docker compose down || true
				docker compose up -d
				echo -e "${GREEN}Compose 项目 [$project_name] 還原完成！${NC}"
			fi
		done
		...
		for json in "$BACKUP_DIR"/*_inspect.json; do
			...
			[[ -z "$IMAGE" || "$IMAGE" == "null" ]] && { echo -e "${RED}未找到鏡像訊息，跳過:$container${NC}"; continue; }
		...
		if [ -f "$BACKUP_DIR/home_docker_files.tar.gz" ]; then
			...
			echo -e "${YELLOW}未找到 /home/docker 下檔案的備份，跳過...${NC}"
		fi
...
docker_tato() {
	...
	if command -v docker &> /dev/null; then
		echo -e "${gl_kjlan}------------------------"
		echo -e "${gl_lv}環境已經安裝${gl_bai}容器:${gl_lv}$container_count${gl_bai}  镜像: ${gl_lv}$image_count${gl_bai}網路:${gl_lv}$network_count${gl_bai}卷:${gl_lv}$volume_count${gl_bai}"
	fi
}
...
if command -v docker &>/dev/null; then
	if docker ps --filter "name=nginx" --filter "status=running" | grep -q nginx; then
		echo -e "${gl_huang}------------------------"
		echo -e "${gl_lv}環境已安裝${gl_bai}  站点: $output  数据库: $db_output"
	fi
fi
...
linux_ldnmp() {
  while true; do
	...
	echo -e "${gl_huang}27.  ${gl_bai}安裝AI繪畫提示詞產生器${gl_huang}28.  ${gl_bai}站点反向代理-负载均衡"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}9.   ${color9}Poste.io郵件伺服器程式${gl_kjlan}10.  ${color10}RocketChat多人在线聊天系统"
	  echo -e "${gl_kjlan}------------------------"
	  ...
	  echo -e "${gl_kjlan}39.  ${color39}Bililive直播录制工具                ${gl_kjlan}40.  ${color40}webssh網頁版SSH連線工具"
	  echo -e "${gl_kjlan}------------------------"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "系統工具"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}1.   ${gl_bai}设置脚本启动快捷键                 ${gl_kjlan}2.   ${gl_bai}修改登入密碼"
	  ...
	  echo -e "${gl_kjlan}27.  ${gl_bai}红帽系Linux内核升级                ${gl_kjlan}28.  ${gl_bai}Linux系統核心參數優化${gl_huang}★${gl_bai}"
	  echo -e "${gl_kjlan}29.  ${gl_bai}病毒掃描工具${gl_huang}★${gl_bai}                     ${gl_kjlan}30.  ${gl_bai}文件管理器"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}31.  ${gl_bai}切换系统语言                       ${gl_kjlan}32.  ${gl_bai}命令列美化工具${gl_huang}★${gl_bai}"
		  ...
		  66)
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  set_dns
				  echo -e "[${gl_lv}OK${gl_bai}] 8/10. 自動最佳化DNS位址${gl_huang}${gl_bai}"
		...
		case "$Limiting" in
		   ...
		   24) # 复制文件目录
			 ...
			 25) # 传送文件至远端服务器
				...
				read -e -p "請輸入遠端伺服器使用者名稱 (預設root):" remote_user
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装科技lion脚本         ${gl_kjlan}12. ${gl_bai}更新系統${gl_kjlan}13. ${gl_bai}清理系統"
